...
- **Public release** (`push` to `v*` tag or `workflow_dispatch` without `dry_run`): requires all secrets above. The workflow verifies presence before building, generates `release/electron-builder.release.generated.yml` from `DEVELOPER_ID_APP_IDENTITY`, and calls `electron-builder` with the release config.
