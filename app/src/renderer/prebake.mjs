 ...
 *
 * Adapted from the upstream Strudel REPL prebake module. The website version also imports
 * ./files.mjs, which loads local samples through the Tauri filesystem API, and
 * ./idbutils.mjs for browser-stored sample banks. Neither applies here.
 */
export async function prebake() {
  await Promise.all([
    ...
    samples(`${baseCDN}/uzu-drumkit.json`, `${baseCDN}/uzu-drumkit/`, { prebake: true, tag: 'drum-machines' }),
    samples(`${baseCDN}/uzu-wavetables.json`, `${baseCDN}/uzu-wavetables/`, { prebake: true }),
