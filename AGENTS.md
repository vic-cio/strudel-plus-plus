...
- Floating panels of both classes portal into `.app-overlay` (rendered by `App.tsx`; `.app` is its positioned ancestor) — one coordinate space spanning the whole app surface, so drags clamp to the overlay's bounds, never to `.editor-viewport`. Releasing over the dock docks either class: session panels through the shared `dockPanel` path in `PluginDock.tsx` (the floating close button uses it too), function instances to `placement: { kind: 'docked' }` in renderer state only. `isOverDock` compares live pointer coordinates with the dock rect because the dragged panel sits above the dock; a press that never passes `DRAG_THRESHOLD` is a click, not a drop, so a panel resting on the dock cannot dock itself by being focused.
- `dockPanel` must delete the `floating` key when the last panel leaves it: `{ ...state }` carries the old array, and a leftover entry resurrects the closed panel next to its reattached tab (the "closing creates a clone" failure). The add menu's candidates exclude ids that are floating for the same reason — a plugin open as a panel must not be offered again.
