...
## Settings page (overlay, never a replacement)

- `SettingsPage` renders as a full-viewport OVERLAY on the mounted app (`App.tsx` builds `settingsOverlay` after the main tree), never an early-return swap. The editor, playing scheduler, beat selection, dirty markers, and plugin state must survive a Settings round trip. The old early-return unmounted the editor and a fresh StrudelMirror came back with `initialCode: '// loading'` and nothing to replace it — that regression is pinned by the App settings-overlay tests.
- Every change saves instantly through typed settings IPC (`settings:update`) and reaches live surfaces via `onSettingsChange` → `applySettings` in `App.tsx` (record mode, beat-switch latency, close behavior). The FileTree latency dropdown persists through `settings.update` on change too, so the sidebar and the Settings page cannot drift apart.
- The sessions-root pointer is displayed from `sessions:rootStatus`, not the vestigial `Settings.sessionsRoot` field. `chooseRoot` still only works while no session is open (main's contract: re-rooting must not disturb the active store/watcher/harness); Settings surfaces that refusal inline instead of a dead button. Settings-page update failures show in the overlay's own footer — the status bar is underneath it.

## Close lifecycle

- Closing is the renderer's decision, because only the renderer sees its drafts. `beforeunload` is only the interception seam — `event.returnValue` shows no dialog in Electron, which was the "close button does nothing" trap. `closeBehavior` decides: `ask` opens `CloseDialog` (Save all / Discard / Cancel; failed saves and conflicts are listed and never overwritten); `auto-save` runs `saveAllDrafts` silently, then closes — or stays open with the failures visible; `discard` lets the close pass at once.
- A confirmed choice sets `closeConfirmedRef` in `App.tsx` so `window.close()`'s own re-entrant beforeunload passes; `closeBusyRef` keeps a second close attempt from starting a second save-all. Main no longer vetoes quit (the old `before-quit` handler's `saveAllTrigger` send had no listener and would dead-lock a quit once `closeDirty` was real); `close:check`/`close:dirty-state` stay honest through `reportDirty` on every draft-state change.
- Cross-session save-all: `saveAllDrafts` (closeCoordinator) writes the ACTIVE session through `beats:write` and every other session's leftover drafts through `beats:writeIn` — main resolves the named session's folder, queues behind session transitions, and refuses missing sessions. Writing another session's draft through `beats:write` would land it in the ACTIVE session's files; `saveAllSessions` alone (single session) is kept for its existing contract.
...
- Floating panels of both classes portal into `.app-overlay` (rendered by `App.tsx`; `.app` is its positioned ancestor) — one coordinate space spanning the whole app surface, so drags clamp to the overlay's bounds, never to `.editor-viewport`. A tab can be dragged out by holding past `DRAG_THRESHOLD`: session tabs detach through `detachSessionTab` (emitting one state update with the floating geometry) and function tabs through `detachDockTab` (setting placement to `{ kind: 'floating', geometry }`). Releasing over the dock docks through pane-aware `resolveDockDropTarget` / `dropFloatingPanel` (`dockDrop.ts`), which highlights only the pane under the pointer (`dock-pane-drop-target`), records the left/right `side`, and auto-splits (`autoSplit`) when only one pane exists; function instances dock to `{ kind: 'docked', paneIndex }` (pane 0 by default, clamped if the pane does not exist). The floating EQ (`def.id === 'eq'`) closes via `closeFloatingPanel` and is removed from both floating and tabs rather than reattached; other session panels dock through the shared `dockPanel` path. A click that never crosses the drag threshold selects the tab, not a drop. `dockPanel` must delete the `floating` key when the last panel leaves it: `{ ...state }` carries the old array, and a leftover entry resurrects the closed panel next to its reattached tab (the "closing creates a clone" failure). The add menu excludes floating ids so an open panel is never offered twice.
- Every numeric edit to a function plugin (`gain()` etc.) is applied with `replaceCodeRange` (`useStrudel`) via a surgical `{ from, to, insert }` change (`rebaseFunctionPluginInstances` in `functionPlugin.ts` updates other instances by delta) instead of a whole-document `setCode`, preventing selection/highlight flicker; only a missing editor falls back to `setCode`. Function instances stay out of `DockState` (they live only in renderer `App` state) and are cleared on beat/session switches.
- The session gain device is labeled `TRIM` under its stable id `gain`: the id is the whole migration, so `.session.json` tabs/pluginState keep resolving after the rename. The function-scope `GAIN` catalog entry is named for the actual `gain()` call and keeps its label; function titles read `GAIN · gain @ line N` from the instance's stored `functionRange`.
...
- `RecordControl` (`app/src/renderer/components/RecordControl.tsx`) is the titlebar transport's one record button (the status bar shows the mode as text, never a second control). It reads `mode`/`source`/`masterAvailable` props — `masterAvailable` is playback-started — and reports `RecordEvent` (`start`/`stop`/`complete`/`fail`). `complete` still passes `filePath: undefined`: the blob is handled externally by design (declined fix `rc-complete-missing-filepath`).
- That external handling is `onRecordEvent` in `App.tsx`: on `stop` it awaits the same take and exports it through `desktop.recording.save`. `recording.ts`'s `stop` is idempotent (one cached promise) so the control's internal await and the export share ONE `MediaRecorder.stop()` — a second stop on an inactive recorder throws and would read as a spurious recording failure. Pinned by `recording.test.ts`.
- Recording mode (`audio`/`mp4`) is a settings-backed preference (`recordConfig.mode` in `app/src/shared/settings.ts`), applied live to the titlebar button via `SettingsPage`'s `onSettingsChange`. Record failures — a missing master mix, a dead recorder, a refused export — surface through `setBeatError` (the tree error strip) with `recordingFailureMessage` labels; a declined save dialog (returns `undefined`) is a choice, not a failure.
- Active recording form is shown through the component's `recording`/`timerSec` state.
